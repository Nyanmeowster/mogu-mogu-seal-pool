...
test("六件道具都使用同一條點按、拖曳、鍵盤與取消路徑", () => {
  ...
  assert.match(script, /class="pool-toy-hit-map" aria-hidden="true"/);
  assert.match(script, /function createAlphaHitRuns\(image, gridWidth, gridHeight\)/);
  ...
  assert.match(script, /paintAlphaHitArea\(\$\("seal"\), image, \$\("seal-hit-map"\), "seal-hit-cell"\)/);
  assert.match(html, /id="seal-hit-map" class="seal-hit-map" aria-hidden="true"/);
  assert.match(styles, /\.pet-seal\s*\{[\s\S]*?pointer-events: none;/);
  assert.match(styles, /\.seal-hit-cell\s*\{[\s\S]*?pointer-events: auto;/);
  assert.match(styles, /\.pool-toy:not\(\.decor-ring\)\s*\{[\s\S]*?pointer-events: none;[\s\S]*?touch-action: pan-y;/);
  assert.match(styles, /#decorations\s*\{[\s\S]*?z-index: 12;[\s\S]*?pointer-events: none;/);
  assert.match(styles, /\.pool-toy-hit-map\s*\{[\s\S]*?pointer-events: none;/);
  assert.match(styles, /\.pool-toy-hit-cell\s*\{[\s\S]*?pointer-events: auto;[\s\S]*?touch-action: pan-y;/);
  assert.doesNotMatch(styles, /\.pool-toy:not\(\.decor-ring\)::before/);
...
test("每種體型都有五張海豹與道具完整接觸的專屬互動圖", () => {
  ...
  assert.doesNotMatch(script, /seal-stage-[1-5]-toy-(?:ball|kelp|glow|scent|ice)-v2\.webp/);
